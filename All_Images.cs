...
namespace USG
{
    class All_Images : Program
    {
        ...
        public string All_Player_Items(bool Double_Barrel_Insert = false, bool Double_Barrel_After_Insert = false)
        {
            ...
            {
                return "сводка об предметах (чтобы их использовать, жми на цифру на клавиатуре, соответсвующую цифре предмета): 1 - использовать +хп, 2 - использовать наручники (соперник пропустит ход, т.е. вы походите два раза), 3 - инвентор (меняет тип текущего/щих патрона/ов). 4 - выйти из этого меню. у тебя есть: ";
            }
            else
            {
                return "сводка об предметах (чтобы их использовать, жми на цифру на клавиатуре, соответсвующую цифре предмета): 1 - использовать +хп, 2 - использовать наручники (соперник пропустит ход, т.е. вы походите два раза), 3 - использовать патрончекер (покажет тип патрона), 4 - рандомный патрончекер (укажет тип случано выбранного патрона), 5 - инвентор (меняет тип текущего/щих патрона/ов). 6 - выйти из этого меню. у тебя есть: ";
            }
        }
        ...
        public string Player_Menu(string Player_Name, int Count_Of_Player_Lives, string Opponent_Name, int Count_Of_Items, bool Opponent_HandCuffed = false)
        {
            if (Count_Of_Items == 0)
            {
                return $"Ходит {Player_Name}. перед вашим лицом {Opponent_Name}. кол-во ваших жизней {Count_Of_Player_Lives}. 1 - прокрутить оружие на столе, 2 - выстрел в себя, 3 - выстрел в соперника.                                                                                                                                                                                                                       ";
            }
            else if (Opponent_HandCuffed)
            {
                return $"Ходит {Player_Name}. перед вашим лицом {Opponent_Name}, у него связаны руки. кол-во ваших жизней {Count_Of_Player_Lives}. 1 - прокрутить оружие на столе, 2 - выстрел в себя, 3 - выстрел в соперника, 4 - меню предметов.                                                                                                                                                                                               ";
            }
            else
            {
                return $"Ходит {Player_Name}. перед вашим лицом {Opponent_Name}. кол-во ваших жизней {Count_Of_Player_Lives}. 1 - прокрутить оружие на столе, 2 - выстрел в себя, 3 - выстрел в соперника, 4 - меню предметов.                                                                                                                                                                                                                       ";
            }
        }
    }
}
